...
public sealed class WebhookFulfillmentTests : IClassFixture<IntegrationTestWebApplicationFactory>
{
    ...
    public async Task Webhook_ShouldMarkOrderPaid_AndIssueLicense_Once()
    {
        ...
        _factory.PayKryptClient.Intents[intentId] = new EstherLink.Backend.Services.Commerce.PayKryptIntentResponse
        {
            Id = intentId,
            Status = "confirmed",
            Amount = "149.00",
            Currency = "USD",
            TransactionsSummary = new EstherLink.Backend.Services.Commerce.PayKryptTransactionsSummary
            {
                IsFullyPaid = true,
                OutstandingFiat = "0"
            }
