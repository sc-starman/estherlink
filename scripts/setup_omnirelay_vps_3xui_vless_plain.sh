...
Commands:
  ...
  sync-clients Compatibility no-op (vless/3x-ui manages clients via API)
...
Options:
  ...
  --gateway-sni <host>          Compatibility option (ignored for plain VLESS)
  --gateway-target <host:port>  Compatibility option (ignored for plain VLESS)
  ...
  --bootstrap-mode <mode>       tunnel | direct (default: ${BOOTSTRAP_MODE})
...
verify_bootstrap_socks() {
  ...
  if ! omnirelay_bootstrap_verify_egress "$BOOTSTRAP_MODE" "$BOOTSTRAP_SOCKS_PORT" "$PROXY_CHECK_URL" 24 5; then
    ...
    die "Bootstrap SOCKS endpoint is not reachable or egress failed on 127.0.0.1:${BOOTSTRAP_SOCKS_PORT}. Ensure Windows relay/tunnel are running and sshd permits remote forward for this port."
  fi

  if [[ "$BOOTSTRAP_MODE" == "direct" ]]; then
    progress 14 "Validated direct VPS internet egress"
  else
...
install_packages_online() {
  if [[ "$BOOTSTRAP_MODE" == "direct" ]]; then
    progress 20 "Configuring apt for direct VPS network"
  else
    progress 20 "Configuring apt to use SOCKS bootstrap"
  fi
  configure_apt_proxy
  if [[ "$BOOTSTRAP_MODE" == "direct" ]]; then
    progress 22 "Syncing VPS clock over direct network (if needed)"
...
find_existing_managed_inbound_id() {
  local api_base="$1"
  local list_resp existing_id existing_proto existing_security
  ...
  if [[ -n "$existing_id" && "$existing_id" != "null" ]]; then
    ...
    existing_security="$(printf '%s' "$list_resp" | jq -r --arg id "$existing_id" '
      def parse_stream:
        if . == null then {}
      ...
      if .success != true then
        ...
        ((.obj // []) | map(select((.id|tostring) == $id)) | .[0]) as $inbound
        | ($inbound.streamSettings | parse_stream | .security // empty)
      end
    ' 2>/dev/null || true)"

    if [[ "$existing_proto" == "vless" && ( "$existing_security" == "none" || -z "$existing_security" ) ]]; then
      INBOUND_ID="$existing_id"
      return 0
    fi

    log "Inbound on port ${PUBLIC_PORT} is protocol='${existing_proto:-unknown}' security='${existing_security:-unknown}', replacing with managed VLESS inbound."
...
provision_managed_inbound() {
  progress 78 "Creating managed VLESS inbound"
  local api_base client_uuid sub_id
  ...
  if [[ "$success" != "true" ]]; then
    ...
    if [[ "$add_msg" == *"Duplicate email"* ]]; then
      ...
      settings_json='{"clients":[],"decryption":"none","encryption":"none"}'
      add_resp="$(curl --noproxy '*' --fail --silent --show-error --insecure --cookie "$XUI_COOKIE_JAR" \
        --header 'Content-Type: application/x-www-form-urlencoded; charset=UTF-8' \
        --data-urlencode "up=0" \
        --data-urlencode "down=0" \
...
OMNIRELAY_ACTIVE_PROTOCOL=vless_plain_3xui
...
write_gateway_metadata() {
  ...
  jq -n \
    --arg activeProtocol "vless_plain_3xui" \
...
sync_clients_impl() {
  require_root
  log "sync-clients is not required for vless_plain_3xui; skipping."
  echo "ok"
}

get_protocol_impl() {
  echo "vless_plain_3xui"
}
