    ...
    public async Task<(bool Success, string Message)> TestAsync(ServiceConfig config, CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ...
            try
            {
                ...
                // ConnectTimeout is 10s in SshTunnelProcessFactory; wait beyond that
                // before treating a still-running ssh process as a successful connection.
                ...
                if (completed == waitTask)
                {
                    ...
                    if (!attemptedRepair && SshKnownHostsRepair.LooksLikeHostKeyMismatch(message))
                    {
                        attemptedRepair = true;
                        var repair = await SshKnownHostsRepair.TryRepairAsync(config, cancellationToken);
                        _fileLog.Warn($"Detected SSH host-key mismatch during tunnel test. {repair.Message}");
                        if (repair.Success)
                        {
                            continue;
                        }
