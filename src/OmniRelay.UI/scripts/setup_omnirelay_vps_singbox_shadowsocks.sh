...
command_install() {
  ...
  connector_merge_metadata_json "$(jq -c -n --arg clientsFile "$PROTOCOL_CLIENTS_FILE" --arg runtimeFile "$PROTOCOL_RUNTIME_FILE" '{shadowsocks:{clientsFile:$clientsFile,runtimeFile:$runtimeFile},accounting:{source:"singbox_log",clientsFile:$clientsFile}}')"
