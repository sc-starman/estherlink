...
func withFileLock(_ string, fn func() error) error {
	// Windows service executes connector-core per relay in a single process context.
	// We keep lock semantics as no-op on Windows to avoid Linux-only flock dependency.
	return fn()
}
...
func repairAccountingPermissions(dbPath string, _ string) error {
	// No POSIX group/chown model on Windows.
