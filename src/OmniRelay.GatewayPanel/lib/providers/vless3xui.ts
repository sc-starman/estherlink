...
export class VlessReality3xuiProvider implements GatewayProtocolProvider {
  public readonly protocolId = "vless_reality_3xui";
  ...
  public async updateClient(session: OmniSession, client: GatewayClientRecord): Promise<void> {
    ...
    const payload: GatewayClientRecord = { ...client, flow: "xtls-rprx-vision" };
    const form = new URLSearchParams();
    form.set("id", inboundId);
  ...
  public async buildClientConfig(session: OmniSession, request: Request, clientId: string): Promise<ClientConfigPayload> {
    ...
    if (!sni || !shortId || !publicKey) {
      throw new Error("Inbound REALITY settings are incomplete.");
    }
