...
{
    private const string AskPassSecretEnv = "OmniRelay_SSH_SECRET";
    ...
    private static void ConfigureAskPass(ProcessStartInfo startInfo, string secret)
    {
        ...
        startInfo.Environment["DISPLAY"] = "OmniRelay:0";
    ...
    private static string EnsureAskPassLauncher()
    {
        ...
        {
            var scriptContent = "$secret = $env:OmniRelay_SSH_SECRET; if ($null -ne $secret) { [Console]::Out.Write($secret) }";
